// Ported from next16-claude-starter (Textura), adapted for this project.
 ...
 *
 * NOTE: the starter's own default config pairs the ≤640px tier with a
 * baseWidth of 360 — that produces a real discontinuity at the 640/641px
 * boundary (font-size jumps from ~28px down to ~10px between those two
 * pixels). Every other tier uses `baseWidth === maxWidth`, which scales
 * continuously. This project's config follows that same 1:1 pattern on the
 * mobile tier too, so there's no cliff.
 */
